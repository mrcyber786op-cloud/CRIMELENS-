...
export const ForensicAcademy: React.FC<ForensicAcademyProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<
    'webinars' | 'courses' | 'careers' | 'materials' | 'quiz'
  >(initialSubTab);
  ...
  const [selectedWebinar, setSelectedWebinar] =
    useState<ForensicWebinar | null>(null);

  const [registrationSuccess, setRegistrationSuccess] =
    useState(false);

  const [isWebinarSubmitting, setIsWebinarSubmitting] =
    useState(false);
  ...
  const [selectedCourse, setSelectedCourse] =
    useState<ForensicWeekCourse | null>(null);

  const [courseEnrolled, setCourseEnrolled] =
    useState(false);

  const [isCourseSubmitting, setIsCourseSubmitting] =
    useState(false);
  ...
  const [studySearchTerm, setStudySearchTerm] =
    useState('');
  ...
  const [isAnswerSubmitted, setIsAnswerSubmitted] =
    useState(false);
  ...
  const [selectedCareer, setSelectedCareer] =
    useState<CareerPath>(CAREER_PATHS[0]);

  // =========================================================
  // WEBINAR REGISTRATION
  // =========================================================

  const handleWebinarRegister = async (
    e: React.FormEvent
  ) => {
    e.preventDefault();

    if (!selectedWebinar || isWebinarSubmitting) return;
...
Role: ${regForm.role}`
      ...
      alert(
        'Registration submit nahi ho paayi. Please try again.'
      );
    } finally {
  ...
  const handleCourseEnroll = async (
    ...
    if (!selectedCourse || isCourseSubmitting) return;
...
Week Code: ${selectedCourse.weekCode}
Tuition: ${selectedCourse.tuition}
Duration: ${selectedCourse.duration}
Affiliation: ${courseForm.affiliation || 'Not provided'}`
      });
      ...
      alert(
        'Course enrollment submit nahi ho paayi. Please try again.'
      );
    } finally {
  ...
  const handleAnswerSubmit = () => {
    ...
    if (
      selectedOption ===
      FORENSIC_QUIZ_QUESTIONS[currentQuestionIdx].correctIndex
    ) {
  ...
  const handleNextQuestion = () => {
    if (
      ...
      FORENSIC_QUIZ_QUESTIONS.length
    ) {
  ...
  const filteredStudyMaterials =
    STUDY_MATERIALS.filter(m => {
      const search =
        studySearchTerm.toLowerCase();
  ...
  return (
    <section
      id="forensic-academy-section"
      className="py-12 md:py-16 bg-slate-950"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-950/80 border border-blue-800/40 text-blue-300 text-xs font-mono mb-2">
            ...
            <span>
              FORENSIC EDUCATION & CAREER DEVELOPMENT HUB
            </span>
          </div>
          ...
          <p className="mt-2 text-sm text-slate-300 leading-relaxed">
            Professional learning departments for university
            students, law enforcement personnel, private
            investigators, and legal advocates. Master real
            casework tools through specialized webinars,
            intensive forensic week bootcamps, curated study
            materials, and career guidance.
          </p>
        </div>
        ...
        {activeSubTab === 'courses' && (
          <div className="mt-8 space-y-6">
            ...
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 text-xs">

              <span className="text-slate-300">
                ⭐ <strong>CRIMELENS Forensic Weeks</strong> are
                intensive 5-day immersive bootcamps with
                practical virtual labs, write-blocked hard drive
                exercises, and certificates of completion.
              </span>
              ...
              <div className="lg:col-span-4 space-y-2">
                ...
                <div className="mt-4 p-4 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-300">
                  ...
                  <p className="text-slate-400 text-[11px] leading-relaxed">
                    CRIMELENS pairs university forensic students
                    with active casework examiners for mock trial
                    prep and thesis data collection.
                  </p>
              ...
              <div className="lg:col-span-8 rounded-xl bg-slate-900/80 border border-slate-800 p-6 space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="p-3.5 rounded-lg bg-slate-950/60 border border-slate-800">
                    ...
                    <ul className="space-y-1.5">
                      ...
                      {selectedCareer.requiredEducation.map(
                        (edu, idx) => (
                          <li
                            key={idx}
                  ...
                  <div className="p-3.5 rounded-lg bg-slate-950/60 border border-slate-800">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {selectedCareer.keyCertifications.map(
                        (cert, idx) => (
                          <span
                            key={idx}
                            className="text-[11px] font-mono px-2 py-0.5 rounded bg-blue-950 text-blue-300 border border-blue-800"
                          >
                            {cert}
                          </span>
                        )
                ...
                <div className="p-3.5 rounded-lg bg-slate-950/60 border border-slate-800 text-xs">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedCareer.typicalWorkplaces.map(
                      (place, idx) => (
                        <span
                          key={idx}
                          className="text-[11px] px-2.5 py-1 rounded bg-slate-800 text-slate-200 border border-slate-700"
                        >
                          {place}
                        </span>
                      )
          ...
          <div className="mt-8 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search protocols, handwriting points, hash guides..."
              ...
              >
                ...
                <option value="First Responder Guide">
                  First Responder Guide
                </option>
                <option value="Digital Forensics Cheatsheet">
                  Digital Forensics Cheatsheet
                </option>
                <option value="Questioned Documents">
                  Questioned Documents
                </option>
                <option value="Legal & Evidence Law">
                  Legal & Evidence Law
                </option>
              </select>
            ...
            {!quizFinished ? (
              ...
              <div className="rounded-xl bg-slate-900 border border-slate-800 p-8 text-center shadow-2xl">
                ...
                <p className="text-sm text-slate-300 mt-2">
                  ...
                  out of {FORENSIC_QUIZ_QUESTIONS.length}
                  on the Forensic Standards Self-Assessment.
                </p>

                <p className="text-xs text-slate-400 max-w-md mx-auto mt-2">
                  {score >= 4
                    ? 'Outstanding! You have a solid grasp of forensic standards and methodology.'
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative">
            ...
            {!registrationSuccess ? (
              ...
              <form
                onSubmit={handleWebinarRegister}
                className="space-y-4"
              >
                ...
                <div className="p-3 rounded bg-slate-950 border border-slate-800 text-xs font-mono text-slate-300">
                  ...
                  <div>
                    Speaker: {selectedWebinar.speaker}
                  </div>
                ...
                <div className="pt-2 flex justify-end gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={isWebinarSubmitting}
                    className="px-5 py-2 rounded bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-xs font-semibold shadow"
                  >
                    {isWebinarSubmitting
                      ? 'Submitting...'
                      : 'Submit Registration'}
              ...
              <div className="text-center py-4 space-y-3">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  ...
                  successfully submitted to CRIMELENS.
                  Our team will contact you at{' '}
                  <strong className="text-cyan-300">
                    {regForm.email}
                  </strong>{' '}
                  with webinar access details.
                </p>
            ...
            {!courseEnrolled ? (
              ...
              >
                ...
                <div className="space-y-3 text-xs">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="University, Police Dept, or Private Practice"
                      value={courseForm.affiliation}
                      onChange={e =>
                        setCourseForm({
                          ...courseForm,
                ...
                <div className="pt-2 flex justify-end gap-2">
                  ...
                  <button
                    ...
                    className="px-5 py-2 rounded bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white text-xs font-semibold shadow"
                  >
                    {isCourseSubmitting
                      ? 'Submitting...'
              ...
              <div className="text-center py-4 space-y-3">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  Your request for{' '}
                  <strong className="text-cyan-300">
                    {selectedCourse.weekCode}
                  </strong>{' '}
                  has been successfully submitted to
                  CRIMELENS. Our team will contact you
                  regarding seat confirmation, payment,
                  and course details.
                </p>
      ...
      {selectedStudyMaterial && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm animate-in fade-in">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl relative overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs sm:text-sm text-slate-300 leading-relaxed font-sans">
              ...
              <div className="p-3.5 rounded-lg bg-slate-950 border border-slate-800 text-xs font-mono text-slate-400 flex flex-wrap gap-4">
                ...
                <span>
                  Category:{' '}
                  <strong className="text-cyan-300">
                    {selectedStudyMaterial.category}
                  </strong>
                </span>

                <span>
                  Format: {selectedStudyMaterial.format}
                </span>

                <span>
                  Est. Reading:{' '}
                  {selectedStudyMaterial.readTime}
                </span>
            ...
            <div className="p-4 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-950/60 text-xs">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    ...
                    a.download =
                      `${selectedStudyMaterial.title.replace(
                        /[^a-zA-Z0-9]/g,
                        '_'
                      )}.md`;
